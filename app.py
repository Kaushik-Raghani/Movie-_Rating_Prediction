...
directors = st.text_input("Enter Directors")
genres = st.text_input("Enter Genres")
synopsis = st.text_input("Enter The Movie Synopsis")

if st.button("Predict Rating"):
    with st.spinner("Predicting..."):
        try:
            # Compute actor and director averages
            input_avg_actor = np.mean([avg_actor_rating.get(actor.strip(), np.nan) for actor in actors.split(",")])
            input_avg_director = np.mean([avg_director_rating.get(director.strip(), np.nan) for director in directors.split(",")])

            # Compute overall mean rating
            overall_mean_rating = np.mean(list(avg_actor_rating.values()) + list(avg_director_rating.values()))

            # Handle NaN values
            input_actor_avg = input_avg_actor if not np.isnan(input_avg_actor) else overall_mean_rating
            input_director_avg = input_avg_director if not np.isnan(input_avg_director) else overall_mean_rating

            # Encode synopsis
            synopsis_embedding = sentence_model.encode([synopsis]).reshape(1, -1)

            # Genre vectorization
            input_genres = [genre.strip() for genre in genres.split(",")]
